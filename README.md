...
The printf function sends formatted output to stdout. A custom _printf() for learning purposes was developed by cohort #8 students. _printf() function format string is a character string, beginning and ending in its initial shift state, if any. These arguments are placed using the percentage '%' operator.
...
Many other specifiers and flags were added, and by combining those, the _printf() function generates different output. The following list are the specifiers and flags allowed.
...
|`printf_sting.c` | Function that calls a string type variable. | `None` |
|`printf_bin.c` | Function that gets the binary | `None` |
`printf_oct.c` | Functions that return octal number. | `None` |
`printf_hex.c` | Calls hexadecimal numbers (lowercase). | `None` | 
`printf_HEX.c` | Calls hexadecimal numbers (Uppercase). | `None` |
`printf_unsigned.c` | Returns an unsigned data type. | `None` |
